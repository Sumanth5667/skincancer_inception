...
def combine_files(output_path, input_dir, file_base, file_ext):
    ...
    Args:
        output_path (str): Path to the output combined file.
        input_dir (str): Directory containing the chunk files.
        file_base (str): Base name of the chunk files.
        file_ext (str): Extension of the chunk files.
    """
    with open(output_path, 'wb') as output_f:
        chunk_num = 0
        while True:
            chunk_file = os.path.join(input_dir, f"{file_base}_part{chunk_num}{file_ext}")
...
user_location = st.text_input("Enter your location (e.g., country)", placeholder="country")
